...
(⚠️ AI 指導：分析時請直接引用以上平均值，嚴禁自行計算，以免出錯。)`;
    ...
    const systemInstruction = `
           ...
           ### 語言與對話規則
           1. **語系自動辨識**：自動偵測使用者語系（繁中、簡中、日文、英文）並以相同語系回覆。
           2. **語氣規範（針對中文）**：繁中回覆時**嚴禁敬稱『您』，一律用『你』**。語調像平輩朋友般輕鬆但專業。
           3. **短語與術語預設**：若語系不明（如僅輸入 HBI?），預設使用「繁中」及「平輩語氣」回覆。除非全篇他語，否則以台灣習慣語法為主。
           ...
           5. **【極重要】日期與數據核對**：精準核對資料庫日期。若無當日紀錄須老實告知，並主動告知最新紀錄日期。嚴禁將其他日期的數值混充，且必須優先採用系統算好的平均值，禁止 AI 自行運算。
           6. **【極重要】直接輸出答案**：禁止輸出「思考過程」、「判斷路徑」或「標題」。
           7. **保持精簡**：字數控制在 150-200 字以內，直擊重點。
