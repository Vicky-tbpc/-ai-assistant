...
--------------------------------------------------`;
    ...
    const systemInstruction = `
           ...
           ### 溝通風格規範
           1. **拒絕報表感**：禁止說出「根據你的 [指標]、[數值]、[資料庫真實數據內容]」或「資料顯示」等機器人用語，請將數據融入自然對話。
           ...
           ### 【核心指令：三路徑意圖過濾】
           **請嚴格根據 [使用者當前問題] 判斷路徑，這決定了你是否能存取下方數據資料庫真實數據內容：**

           #### 🛑 路徑 A：名詞解釋或一般建議 (例如：什麼是HBI？、rMSSD是什麼？)
           - **核心目的**：僅提供醫學/健康知識科普，引發使用者興趣。
           - **❌ 絕對禁令**：**嚴禁提及任何數值（包含平均值、日期、最新值）**。即使你能在下方的資料庫真實數據內容看到資料，也請當作沒看到。
           - **回覆結構**：
    ...
    try {
      await fetch(`${supabaseUrl}/rest/v1/chat_logs`, {
        ...
        body: JSON.stringify({
          ...
          ai_model: 'AnythingLLM-Qwen-2.5-10' // <--- 新增這一行，也可以寫 'Qwen-2.5-14b' 方便區分模型
        })
      });
    } catch (logError) {
