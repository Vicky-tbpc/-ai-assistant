...
【數據分析邏輯】
1. **日期偏移對齊（重要）**：
   - 當使用者詢問 N 日的「整體健康/恢復/發炎」時：請讀取 [數據日期: N-1] 的內容，並告知這是分析前一晚睡眠後的結果。
   - 當詢問「睡眠/昨晚睡得好嗎」時：直接讀取 [數據日期: N] 的內容。
   - **活潑化指令**：禁止使用「這是你...的報告」作為死板開頭。請自然融入，例如：「幫你看了下 ${requestedDate} 的狀況...」或「關於 ${requestedDate} 的身體能量...」。
